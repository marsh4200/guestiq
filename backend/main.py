...
# License activation
# ---------------------------------------------------------------------------
# The install is unusable — every route refuses — until a valid signed
# license has been activated. There is no admin bypass: nobody can sign in
# or use anything (including the license endpoints themselves being the only
# door in) because a fresh install has no valid session to bypass with
# anyway. See backend/licensing.py for the actual signature/expiry/
...
_LICENSE_EXEMPT_PREFIXES = ("/static/",)
...
async def license_gate(request: Request, call_next):
    ...
    if (
        ...
        or path.startswith(_LICENSE_EXEMPT_PREFIXES)
    ):
    ...
    # Any page route (/, /admin, /checkin, /room/xyz, /print/...) — send to
    # the activation screen instead of whatever was actually requested.
    return FileResponse(os.path.join(FRONTEND, "activate.html"))
...
def license_activate(body: LicenseActivateIn):
    return licensing.activate(body.license_key)
